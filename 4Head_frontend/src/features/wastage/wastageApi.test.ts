...
describe("wastageApi contracts", () => {
  ...
  it("posts Wastage shrinkage to the dedicated backend endpoint", async () => { const body = { quantityKg: 1.5, ratePerKg: 300, reason: "spoilage" as const, writeoffDate: "2026-07-15" }; await dispatch(api.endpoints.createWastageStockWriteoff.initiate(body)); expect(requests[0]).toMatchObject({ url: "/wastage/stock/writeoffs", method: "POST", body }); });
